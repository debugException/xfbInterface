...
@org.springframework.stereotype.Service("syBadInfoBlackListQuery")
public class SyBadInfoBlackListQuery {
	
	private static Logger log = Logger.getLogger(SyBadInfoBlackListQuery.class);
	
	
	@Autowired
	private IsybadInfoService  sybadInfoService ;
	/**
	 * 查询成功状态返回码
	 * */
	 ...
	 * 请求编码 第一个数据源
	 * */
	private final static String serviceCode ="SyBadInfoAction";
	 ...
	 */
	public  String blackListQuery(String name,String idcode,String userId,String loginName)  {
  		...
  		if(null!=name&&!"".equals(name)&&null!=idcode&&!"".equals(idcode)){
			...
			String requestparameter = "";
				//0代表查询类别   身份证查询
					try {
						requestparameter = getRequestParameter(name, idcode);
					}catch (Exception e) {
						log.error("查询参数信息错误：" + e);
					}
					String msgString = "" ;
					Document doc = null;
					try {
						msgString = getReturnMsg(requestparameter);
						log.info("------------------------------------------>"+msgString);
						doc = DocumentHelper.parseText(msgString);
					}catch (DocumentException e) {
						log.error("解析返回结果异常：", e);
					}catch (IOException e) {
						log.error("请求接口服务异常：" + e);
					}
					if(null!=doc){
	 ...
	 */
	public  String saveResultDocument(Document doc,String name ,String idcode,String userId,String loginName)  {
		...
		if(QUERY_SUCCESS.equals(resultStr)){
			...
			try {
				resultsENText = DocumentUtil.getDecryptStrByNodePath(doc,"/DATA/RESULTS");
				log.info("查询用户"+name+",身份证号码为"+idcode+"的返回结果为"+resultsENText);
			}catch (Exception e) {
				log.error("获取results节点异常：", e);
			}
			//查询结果results字符串
		}else{
	 ...
	 * <p>Description:黑名单数据保存</p>
	 * @param resultDoc
	 * @param name 姓名 
	 * @param idcode 身份证号
	 * @param resultcode  请求接口名称
	...
	public  void saveResult(Document resultDoc, String name, String idcode, String resultcode,String userId,String loginName){
		  ...
		  try {
			   sybadInfoService.insertSybadInfo(mp);
		} catch (Exception e) {
			log.error("保存返回结果异常", e);
		}
	}
	 ...
	 * @return 返回 map
	 */
	private  Map<String,String> getParameter(String name,String idcode){
	 ...
	 * @throws DocumentException
	 */
	private  String getRequestParameter (String name,String idcode) throws InvalidKeyException, NoSuchAlgorithmException, InvalidKeySpecException, NoSuchPaddingException, IllegalBlockSizeException, BadPaddingException, IOException, DocumentException{
		String parma="";
		Map<String,String> parameter = getParameter(name,idcode);
