...
public class WeChatPayController extends BaseController {
	...
	public String createOrder(Map<String, String> model) throws Exception{
		...
		String orderNo = request.getParameter("trade_no");  //账单流水号
		String money = request.getParameter("total_fee");   //账单支付金额
		...
		try {
			...
			log.info("微信返回的预支付交易会话标识：》》》》》"+prepay_id);
		} catch (Exception e1) {
			e1.printStackTrace();
	  ...
	  public void notifyURL(HttpServletRequest request, HttpServletResponse response) throws Exception{
	    ...
	    if (("SUCCESS").equals(return_code)&&("SUCCESS").equals(result_code)) {
	    	if (sign.equals(signCreate)) {
				String total_fee = map.get("total_fee");  //支付总金额
				String order_no = map.get("out_trade_no");  //交易流水号
   ...
   //更新支付状态等信息
    public void updateStatus(String billNo, String total_fee, String order_no, BillStage billStage) throws Exception{
        ...
        if(memcachedClient.add(""+billId, AppConstants.BILL_TIMEOUT_SECONDS,"")){
            ...
            bill.setRepayChannel(7);//还款类型：7 微信支付
            ...
            wcfCallUtils.callSetBillStatus(billId, optBill.getBillNo(), 2, order_no, optBill.getBillStatus());//设置分期商城账单状态
