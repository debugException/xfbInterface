...
public class QlBlackInfoBlackListQuery {
	...
	public  String blackListQuery(String name,String idcode,String userId,String loginName)  {
		...
		if (null != name && !"".equals(name) && null != idcode
				&& !"".equals(idcode)&&null!=loginName&&!"".equals(loginName)) {
			...
			try {
				...
				log.error("请求参数错误:" + requestparameter);
			}
			...
			try {
				...
				log.error("解析神州融返回结果异常：", e);
			}

			if (null != doc) {
				try {
					result = saveResultDocument(doc, name, idcode, userId,
							loginName);
				} catch (Exception e) {
