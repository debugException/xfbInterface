package cn.emagsoftware.pay.alipay.util;

import cn.emagsoftware.pay.alipay.sign.MD5;
import cn.emagsoftware.pay.alipay.sign.RSA;
import cn.emagsoftware.pay.alipay.util.httpClient.HttpProtocolHandler;
import cn.emagsoftware.pay.alipay.util.httpClient.HttpRequest;
import cn.emagsoftware.pay.alipay.util.httpClient.HttpResponse;
import cn.emagsoftware.pay.alipay.util.httpClient.HttpResultType;
...
public class AlipaySubmit {
	...
	public static String buildRequestMysign(Map<String, String> sPara) {
    	...
    	String mysign = "";
        if(ConfigCache.getSignType().equals("MD5") ) {
        	mysign = MD5.sign(prestr, ConfigCache.getKey(), ConfigCache.getInputCharset());
        }
        if(ConfigCache.getSignType().equals("0001") ){
        	mysign = RSA.sign(prestr, ConfigCache.getKey(), ConfigCache.getInputCharset());
        }
        return mysign;
    }
    ...
    private static Map<String, String> buildRequestPara(Map<String, String> sParaTemp) {
        ...
        if(! sPara.get("service").equals("alipay.wap.trade.create.direct") && ! sPara.get("service").equals("alipay.wap.auth.authAndExecute")) {
        	sPara.put("sign_type", ConfigCache.getSignType());
        }
    ...
    public static String buildRequest(String ALIPAY_GATEWAY_NEW, Map<String, String> sParaTemp, String strMethod, String strButtonName) {
        ...
        sbHtml.append("<form id=\"alipaysubmit\" name=\"alipaysubmit\" action=\"" + ALIPAY_GATEWAY_NEW
                      + "_input_charset=" + ConfigCache.getInputCharset() + "\" method=\"" + strMethod
                      + "\">");
    ...
    public static String buildRequest(String ALIPAY_GATEWAY_NEW, String strParaFileName, String strFilePath,Map<String, String> sParaTemp) throws Exception {
        ...
        //设置编码集
        request.setCharset( ConfigCache.getInputCharset());

        request.setParameters(generatNameValuePair(sPara));
        request.setUrl(ALIPAY_GATEWAY_NEW+"_input_charset="+ ConfigCache.getInputCharset());
...
//            todo 此处需要迭代
    		if(ConfigCache.getSignType().equals("0001")) {
//    			res_data = RSA.decrypt(res_data, ConfigCache.getPrivateKey(),  ConfigCache.getInputCharset());
    			res_data = RSA.decrypt(res_data, ConfigCache.getKey(),  ConfigCache.getInputCharset());
    		}
	...
	public static String query_timestamp() throws MalformedURLException,
        ...
        String strUrl = "https://mapi.alipay.com/gateway.do?service=query_timestamp&partner=" + ConfigCache.getPartner();
