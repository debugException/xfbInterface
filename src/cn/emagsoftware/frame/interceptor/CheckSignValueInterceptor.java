...
public class CheckSignValueInterceptor extends HandlerInterceptorAdapter {
    ...
    static {
        ...
        ignoreUrl.add("/blackList/queryAndInsert.do");
        //交易密码设置
//        ignoreUrl.add("/tradePassword/setTradePassword.do");
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if("/blackList/queryAndInsert.do".equals(request.getServletPath())){
			return true;
		}
		
		if("/customer/resetPassword.do".equals(request.getServletPath())
				|| "/jpushController/jpushMessage.do".equals(request.getServletPath())){
			String flag = request.getParameter("flag");
			if ((!("").equals(flag))&&("xfbManage").equals(flag)) {
				return true;
			}
			if ((!("").equals(flag))&&("xfbJobApp").equals(flag)) {
				return true;
			}
		}
		...
		if(StringUtils.isEmpty(sysType)||StringUtils.isEmpty(sysVersion)){
			...
			if(sysType.equals("Android")){
				...
				if(version.equals(sysVersion)){
					...
					if (!isValidReq) {
						brb.setCodeAndMsg("9988", "您使用的版本过低，请更新最新版本"+version+",感谢您使用信分宝！");
					}
				}
				...
				if(sysVersion.equals(versionIOS[0])||sysVersion.equals(versionIOS[1])){
					...
					if (!isValidReq) {
						brb.setCodeAndMsg("9988", "您使用的版本过低，请更新最新版本"+versionIOS[1]+",感谢您使用信分宝！");
					}
				}
			}
		}
